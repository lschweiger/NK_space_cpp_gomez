...
std::normal_distribution<double> uni(0.7,.09);
...
void NKspacevals_k(vector<double>& input_vec,int k_val){
	int max_values=0;
	if(k_val%2==0) max_values=1<<((k_val+1));
	else max_values=1<<(k_val+1);
    ...
    std::uniform_int_distribution<int> choice(0,(1<<15)-1);
